...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Usuarios disponibles para generar votos de prueba.
        // Cada usuario puede votar una sola vez por cada receta.
        $usuariosParaVotar = [
            $admin,
            $tecnicoAico,
            $tecnicoRedes,
        ...
        foreach ($recetasData as $r) {
            ...
            // Crear la receta SIN votos_util ni votos_no_util.
            $receta = Receta::create($r);

            $recetasCreadas[] = $receta;

            // Entre 0 y 10 votos por receta.
            $cantidadVotos = rand(0, count($usuariosParaVotar));

            if ($cantidadVotos === 0) {
                continue;
            }

            // Elegimos usuarios aleatoriamente sin repetir.
            $usuariosSeleccionados = collect($usuariosParaVotar)
                ->shuffle()
                ->take($cantidadVotos);

            foreach ($usuariosSeleccionados as $usuario) {

                // Aproximadamente 80% útiles y 20% no útiles.
                $tipo = rand(1, 100) <= 80
                    ? 'UTIL'
                    : 'NO_UTIL';
        ...
        Incidente::create([
            ...
            'solucion'     => $recetasCreadas[0]->solucion,
        ]);
        ...
        Incidente::create([
            ...
            'solucion'     => $recetasCreadas[1]->solucion,
        ]);
        ...
        Incidente::create([
            ...
            'solucion'     => $recetasCreadas[3]->solucion,
        ]);
